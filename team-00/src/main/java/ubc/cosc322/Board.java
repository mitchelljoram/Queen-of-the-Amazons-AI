...
public class Board {
	...
	public void updateBoard(ArrayList<Integer> queenPrevPos, ArrayList<Integer> queenNewPos) {
		...
		chosen = (Queen) board[prevRow][prevCol];
		if(chosen != null) {
			chosen.setPrevRow(prevRow);
			chosen.setPrevCol(prevCol);
			chosen.setRow(newRow);
			chosen.setCol(newCol);
			board[newRow][newCol] = board[prevRow][prevCol];
			board[prevRow][prevCol] = null;
			System.out.println("New: "+board[newRow][newCol]);
			System.out.println("Prev: "+board[prevRow][prevCol]);
		}
		else {
			for(Queen queen:enemies) {
				System.out.println("[" +(queen.getRow()+1)+", "+(queen.getCol()+1)+"]");
			}
			for(Queen queen:allies) {
				System.out.println("[" +(queen.getRow()+1)+", "+(queen.getCol()+1)+"]");
			}
		}
	}
