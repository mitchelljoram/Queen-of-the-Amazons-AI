...
public class COSC322Test extends GamePlayer{
    ...
    public void handleGameActionMove(Map<String, Object> msgDetails) {
        ...
        System.out.println(child.getBoard().chosen);
        if(path != null) {
        	if(path.contains(child)) {
            	while(path.get(0)!=child) {
            		path.pop();
            	}
            	node = path.pop();
            }
        ...
        // our move
        System.out.println("-----------------------------------------------------------------");
        for(Queen queen: child.getBoard().allies) {
        	queen.actions.getActions(child.getBoard(), queen);
        }
        for(Queen queen: child.getBoard().enemies) {
        	queen.actions.getActions(child.getBoard(), queen);
        }
